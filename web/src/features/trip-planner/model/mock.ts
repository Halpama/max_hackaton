...
export function createDefaultTripDraft(): TripDraft {
    ...
    const end = shiftDateTime(
        startDate,
        "10:00",
        DEFAULT_TRIP_DURATION_DAYS * 24,
    );
    return {
        destination: "",
        startDate,
...
export const MOCK_ROUTE: RoutePlan = {
    ...
    budgetLabel: "~45 000 ₽",
    cityGuide: {
        type: "Город музеев, воды и парадных фасадов",
        summary:
            "Идеален для длинных прогулок, музеев, набережных и вечерних маршрутов по историческому центру.",
        history:
            "Основан Петром I в 1703 году и более двух столетий был столицей Российской империи.",
        highlights: [
            "Белые ночи и длинный световой день",
            "Музеи лучше планировать с учётом выходных",
            "В центре удобно перемещаться пешком и на метро",
        ],
        tripMonth: 9,
        tripMonthLabel: "Сентябрь",
        seasonality: [2, 2, 3, 4, 5, 5, 5, 5, 4, 3, 2, 2].map(
            (score, index) => ({
                month: index + 1,
                label: [
                    "Янв",
