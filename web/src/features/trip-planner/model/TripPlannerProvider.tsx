...
function applyDraftPatch(patch: Partial<TripDraft>, prev: TripDraft): TripDraft {
...
export function TripPlannerProvider({ children }: { children: ReactNode }) {
  const [draft, setDraft] = useState<TripDraft>(() => createDefaultTripDraft())

  const [activeTripId, setActiveTripId] = useState<string | null>(() =>
    USE_MOCKS ? 'mock' : readActiveTripId(),
  )
  const [route, setRoute] = useState<RoutePlan | null>(USE_MOCKS ? MOCK_ROUTE : null)
  const [routeState, setRouteState] = useState<LoadState>(USE_MOCKS ? 'ready' : 'idle')
  const [routeError, setRouteError] = useState<string | null>(null)

  const [trips, setTrips] = useState<TripSummary[]>(USE_MOCKS ? MOCK_TRIPS : [])
  const [tripsState, setTripsState] = useState<LoadState>(USE_MOCKS ? 'ready' : 'idle')
  ...
  const toggleFavorite = useCallback((placeId: string) => {
    ...
    const request = wasFavorite ? removeFavorite(placeId) : addFavorite(placeId)
    request.catch(() => {
      ...
      setFavoritePlaces((prev) => {
        if (!wasFavorite) return prev.filter((item) => item.id !== placeId)
        const place = resolvePlace()
        if (!place || prev.some((item) => item.id === placeId)) return prev
        return [place, ...prev]
      })
    })
  }, [])
  ...
  const findPlace = useCallback(
    (placeId: string) =>
      route?.places[placeId] ?? favoritePlaces.find((place) => place.id === placeId),
  ...
  useEffect(() => {
    if (USE_MOCKS) {
      ...
      setFavoritePlaces(seeded.map((id) => MOCK_ROUTE.places[id]).filter(Boolean))
      return
    }
