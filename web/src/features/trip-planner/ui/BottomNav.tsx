...
export function BottomNav() {
  ...
  const items = useMemo<NavItem[]>(() => {
    ...
    return [
      ...
      },
      {
        ...
        emphasis: true,
        active: false,
        onSelect: () => navigate(ROUTES.newTrip),
      },
      {
        id: 'favorites',
        label: 'Избранное',
  ...
  useLayoutEffect(() => {
    ...
    const timer = window.setTimeout(releasePill, holdPillRef.current ? 520 : 80)
      ...
      >
        {!closeMode ? (
          <div ref={listRef} className={styles.list} role="tablist">
            <span
              className={pill.ready ? styles.pill : styles.pillHidden}
              style={{
                width: pill.width,
                height: pill.height,
            ...
            {items.map((item, index) => (
              <button
                ...
                aria-label={item.emphasis ? 'Новая поездка' : item.label}
                className={
                  item.emphasis
                    ? styles.emphasis
                    : item.active
