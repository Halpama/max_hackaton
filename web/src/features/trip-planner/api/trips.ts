...
export function createTrip(draft: TripDraft) {
    return api.post<TripCreated>("/api/v1/trips", draft, {
        withInitData: true,
    });
}
...
export function getTrip(tripId: string) {
    return api.get<TripDetails>(`/api/v1/trips/${tripId}`, {
        withInitData: true,
    });
}

export function getTripEditDraft(tripId: string) {
    return api.get<TripEditDraft>(`/api/v1/trips/${tripId}/edit`, {
        withInitData: true,
    });
}

export function updateTrip(tripId: string, draft: TripEditDraft) {
    return api.patch<TripCreated>(`/api/v1/trips/${tripId}`, draft, {
        withInitData: true,
    });
}
    ...
    for (let attempt = 1; attempt <= MAX_STREAM_ATTEMPTS; attempt += 1) {
        if (attempt > 1) {
            await abortableDelay(
                RECONNECT_BACKOFF_MS[attempt - 2] ?? 4_000,
                signal,
            );
        }
            ...
            await streamSse(`/api/v1/trips/${tripId}/stream`, {
                ...
                onEvent: (name, data) => {
                    ...
                    if (name === "error") {
                        settled = true;
                        handlers.onError?.(
                            (data as GenerationErrorEvent).message,
                        );
                    }
                },
            });
